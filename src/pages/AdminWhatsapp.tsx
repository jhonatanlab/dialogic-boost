...
const AdminWhatsapp = () => {
  ...
  const handleDeleteInstanceWebhook = async (inst: { id: string; instance_id: string | null; company_id: string | null; provider?: string | null }) => {
    const provider = (inst.provider as string) || "evolution";
    const isDirectProvider = ["vzaps", "zapster", "evolution"].includes(provider);

    if (!window.confirm("Apagar esta conexão de WhatsApp? Esta ação não pode ser desfeita.")) return;

    try {
      // Conexões diretas (VZaps, Zapster, Evolution) não dependem do n8n.
      if (!isDirectProvider && deleteEndpoint) {
        const response = await supabase.functions.invoke("proxy-n8n", {
          body: {
      ...
      await deleteInstance.mutateAsync(inst.id);
    } catch (error: any) {
      toast({ title: "Erro ao apagar conexão", description: error.message, variant: "destructive" });
    }
  };
