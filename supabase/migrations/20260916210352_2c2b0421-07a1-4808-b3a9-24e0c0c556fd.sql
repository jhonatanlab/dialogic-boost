ALTER TABLE public.whatsapp_instances
  ADD COLUMN IF NOT EXISTS vzaps_client_token text;
...
ALTER TABLE public.whatsapp_instances
  ...
  CHECK (provider = ANY (ARRAY['evolution'::text, 'meta'::text, 'cloud_api'::text, 'zapster'::text, 'vzaps'::text]));

CREATE OR REPLACE FUNCTION public.save_instance_vzaps_config(
  p_instance_id uuid,
  p_vzaps_instance_id text,
  p_instance_token text DEFAULT NULL::text,
  p_client_token text DEFAULT NULL::text,
  p_base_url text DEFAULT 'https://api.vzaps.com'::text
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.save_instance_vzaps_config(uuid, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_instance_vzaps_config(uuid, text, text, text, text) TO authenticated;